...
class My_algorithm_2:
    ...
    def algorithm(depot_distance, distance_list, capacity_v, demand_List, total):#in argomento abbiamo la distanza di tutti i nodi dal deposito, la lista di liste con le distanza di ogni nodo da ogni altro nodo e la capacità del veicolo
       ...
       decrescent_depot = dict(sorted_depot_by_value) #ordinamento in ordine decrescente delle distanze dal deposito
       ...
       while (len(nodes_visitated) < total): #primo ciclo va avanti fino a quando il dizionario delle distanze dal deposito non è nullo
           ...
           print("I nodi visitabili da tale nodo sono (nodo: distanza): ", decrescent_depot)
           ...
           for key in decrescent_depot:  #ciclo per prendere il primo elemento dal dizionario delle distanze dal deposito
               max_node = key
               if int(demand_List[max_node - 1]) <= capacity_v: #vado a prendere il nodo più lontano che ha domanda compatibile con la capacità del veicolo
           ...
           customer.append(max_node) #aggiungo alla lista dei clienti da servire il nodo a distanza massima
           ...
           while total_capacity <= capacity_v: #questo secondo ciclo while va avanti fino a quando non ho raggiunto la capcità massima del veicolo
               ...
               for i in range(len(distance_copy[max_node - 1])):
                   distance_dict[i+1] = distance_copy[max_node-1][i]
