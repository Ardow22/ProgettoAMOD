...
class My_algorithm_3:
    ...
    def algorithm(depot_distance, distance_list, capacity_v, demand_List, total):#in argomento abbiamo la distanza di tutti i nodi dal deposito, la lista di liste con le distanza di ogni nodo da ogni altro nodo e la capacità del veicolo
       demand_copy = copy.deepcopy(demand_List)
       depot_dict = dict()#creazione del dizionario con le domande in modo da poter eliminare i nodi scelti con più facilità
       for i in range(len(demand_copy)):
           depot_dict[i+1] = demand_copy[i]
       sorted_depot_by_value = sorted(depot_dict.items(), key=lambda x: x[1], reverse=True)
       decrescent_depot = dict(sorted_depot_by_value) #ordinamento in ordine decrescente delle domande
       ...
       while (len(nodes_visitated) < total): #primo ciclo va avanti fino a quando il dizionario delle distanze dal deposito non è nullo
           error = 1
           print("Sono nel deposito ed il numero di nodi rimanenti da visitare è: ", len(decrescent_depot))
           print("I nodi visitabili da tale nodo sono (nodo: domanda): ", decrescent_depot)
           ...
           total_capacity = 0
           for key in decrescent_depot:  #ciclo per prendere il primo elemento dal dizionario delle distanze dal deposito
               max_node = key
               if int(demand_List[max_node - 1]) <= capacity_v: #vado a prendere il nodo che ha domanda compatibile con la capacità del veicolo
                   total_capacity = total_capacity + int(demand_List[max_node - 1]) #aggiornamento domanda totale
                   error = 0
                   break
           if (error == 1):
               print("Non ci sono nodi da visitare")
               break
           print("Viene aggiunto ai nodi da visitare il nodo ", max_node)
           print("Aggiornamento domanda totale: ", total_capacity)
           customer.append(max_node) #aggiungo alla lista dei clienti da servire il nodo con domanda massima
           decrescent_depot.pop(max_node) #elimino dal dizionario l'elemento appena inserito nella lista dei clienti da servire
           nodes_visitated.append(max_node) #aggiorno l'elenco con i nodi visitati
           min_node = max_node
